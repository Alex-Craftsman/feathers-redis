...
function routes(app: Application): express.Router {
  ...
  // clear a unique route
  router.get('/clear/single/:key', (req, res) => {
    let target = decodeURIComponent(req.params['key']);
    ...
    if (target.length) {
      ...
      client.get(`${target}`, (err: unknown, reply: unknown) => {
        if (err) {
  ...
  // clear a group
  router.get('/clear/group/:key', (req, res) => {
    let target = decodeURIComponent(req.params['key']);
    ...
    if (target.length) {
      ...
      client.lrange(target, 0, -1, (err: unknown, reply: unknown) => {
        if (err) {
          if (err instanceof Error) {
            res.status(HTTP_SERVER_ERROR).json({
              message: 'something went wrong' + err.message
          ...
          if (reply && Array.isArray(reply) && (reply.length > 0)) {
            ...
            h.clearGroup(target).then(_r => {
              res.status(HTTP_OK).json({
                message:
                  `cache cleared for the group key: ${decodeURIComponent(req.params['key'])}`,
                status: HTTP_OK
              });
            });
          } else {
            ...
            res.status(HTTP_OK).json({
              message:
                `cache already cleared for the group key: ${decodeURIComponent(req.params['key'])}`,
              status: HTTP_NO_CONTENT
            });
          }
        }
